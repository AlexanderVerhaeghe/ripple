...
def my_phenomP_test(phi_ref=0, s1z=0, s2z=0, incl=np.pi/2):
    dist_mpc = 1
    m1_test = 6.0 
    m2_test = 3.0 
    f_ref = 32
...
def random_match_waveforms_debug2(n=1000):
    # Get a frequency domain waveform
    thetas = []
    matches = []
    f_ASD, ASD = np.loadtxt("O3Livingston.txt", unpack=True)
    ...
    for i in tqdm(range(n)):
        ...
        approximant2 = lalsim.SimInspiralGetApproximantFromString("IMRPhenomD")

        hp2, hc2 = lalsim.SimInspiralChooseFDWaveform(
            theta[0]*lal.MSUN_SI,
            theta[1]*lal.MSUN_SI,
            0,
            0,
        ...
        mask_lal = (freqs >= f_l) & (freqs < f_u)
        h0_lalsuite = 2.0 * hp.data.data[mask_lal]
        h0_ripple = 2.0 * hp2.data.data[mask_lal]
        matches.append(
            get_match_arr(
                pad_low,
                pad_high,
